...
export default function QuizScreen() {
  ...
  const { quiz, loading } = useLessonQuiz(aulaId);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>Quiz da lição</Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          <Ionicons name="help-circle-outline" size={56} color={theme.colors.textSecondary} />
          <Text style={styles.empty}>Esta lição ainda não tem quiz.</Text>
        </View>
      ) : (
        <QuizRunner quiz={quiz} onDone={() => router.back()} />
      )}
    </SafeAreaView>
  );
}
