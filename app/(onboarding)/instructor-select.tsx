...
const INSTRUCTORS = [
  {
    ...
    image: require('../../assets/instructors/marinha/objetivo/instrutor-objetivo-marinha-selecao.png'),
    color: '#1F6FEB', // Blue
  ...
  },
  {
    ...
    image: require('../../assets/instructors/marinha/estrategico/instrutor-estrategico-marinha-selecao.png'),
    color: '#238636', // Green
  ...
  },
  {
    ...
    image: require('../../assets/instructors/marinha/didatica/instrutor-didatica-marinha-selecao.png'),
    color: '#A371F7', // Purple
...
const InstructorCard = ({ item, index, scrollX }: { item: typeof INSTRUCTORS[0], index: number, scrollX: Animated.SharedValue<number> }) => {
    ...
    if (session?.user?.id) {
      try {
        await supabase
          .from('profiles')
          .update({
            instructor_profile_id: selectedInstructor.id,
            onboarding_completed: false // Ensures they go through confirm
          })
