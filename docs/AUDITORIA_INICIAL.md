...
**Correção correta:** view **`security definer`** (roda como owner, projeta sem `correta`), GRANT só na view. **Aplicado à `v_c9_simulado_execucao`** (nova, deste trabalho). **PENDENTE (UI phase):** `v_c9_quiz_execucao` e `v_c9_quiz_resultado` (pré-existentes) precisam do mesmo tratamento antes da UI de quiz — `v_c9_quiz_resultado` com cuidado extra (hoje depende do RLS "own attempts"; como definer precisará filtrar `recruta_id = auth.uid()` explicitamente para não expor tentativas de outros). O `rpc_c9_submit_attempt` já é `SECURITY DEFINER` (avalia gabarito server-side) — não afetado.
